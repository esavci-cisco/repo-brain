...
4. **Generate OpenCode integration** — writes `/q` and `/scope` commands, a session-start plugin, and patches `opencode.json` to load the repo map.
...
The repo map is a compact AST skeleton generated by Tree-sitter. It lists every file, class, and function signature — no bodies, no implementation details. The map targets ~2K tokens to stay within budget.
...
| `opencode.json` (patched) | Adds `repomap.md` to `instructions` array | Yes |
...
2. **Check opencode.json has the instructions entry:**
   ```bash
   cat opencode.json
   # Should contain: "instructions": [".repo-brain/repomap.md"]
   ```
...
**OpenCode doesn't seem to know about the codebase**: Check that `opencode.json` has `".repo-brain/repomap.md"` in its `instructions` array and that the file exists. Run `repo-brain generate-opencode` to re-patch.
