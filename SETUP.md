...
repo-brain provides **on-demand context injection** for OpenCode through custom commands, backed by a local vector database and dependency graph.

**Key features:**
- **`/scope <task>`** — blast-radius analysis before you start: affected services, key files, dependencies, risks. **Use this first for 48% faster task completion and 66% fewer tokens.**
- **`/q <query>`** — semantic code search: returns top 3 relevant code chunks
- **Repo map** — Tree-sitter AST skeleton used internally by `/q` and `/scope` for structural awareness
- **Dependency graph** — parsed from `compose.yml`, `pyproject.toml`, proto files. Powers blast-radius analysis.

All data is stored locally in `~/.repo-brain/`. No Docker, no external services, no cloud, no API costs (embeddings run locally).
...
After setup, open OpenCode in the repo and use **`/scope <task>`** before implementing any task for best results.
...
2. `/q` and `/scope` commands use the vector database and repo map for structural awareness
3. The plugin refreshes the map on `session.created` so it stays current

**Important:** The repo map is NOT auto-loaded into the system prompt (that would waste tokens by sending it with every message). Instead, `/q` and `/scope` provide targeted, on-demand context.
...
|------|---------|------------|
| `.repo-brain/repomap.md` | AST skeleton used by `/q` and `/scope` commands | No (gitignored) |
| `.opencode/commands/q.md` | `/q` custom command for semantic search | No (gitignored) |
| `.opencode/commands/scope.md` | `/scope` custom command for task scoping | No (gitignored) |
| `.opencode/commands/summarize.md` | `/summarize` custom command (optional) | No (gitignored) |
| `.opencode/plugins/repo-brain.ts` | Auto-refreshes map on session start | No (gitignored) |
| `opencode.json` | Verified/created if needed | Yes |
...
**`/scope` or `/q` commands not working**: Check that `.repo-brain/repomap.md` exists and that the vector database has been indexed (`repo-brain index`). The commands use the vector database for semantic search, not the repomap file directly.
